...
import { X, Wrench, Eye, EyeOff, Copy, CheckCircle2, AlertTriangle } from 'lucide-react';
...
export default function CreateMechanicModal({ onClose, onCreated }) {
  ...
  const handleSubmit = async (e) => {
    ...
    setError('');
    try {
      await api.users.inviteUser(form.email, 'tech');
      ...
      setSuccess({ email: form.email, temp_password: form.temp_password, full_name: form.full_name });
      ...
      <div className="w-full max-w-lg bg-white rounded-2xl shadow-2xl">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-slate-200">
          <div className="flex items-center gap-2">
            ...
            <div>
              <h2 className="text-base font-black text-slate-900">Create Mechanic Account</h2>
              <p className="text-xs text-slate-500">Admin-provisioned mechanic / tech login</p>
            </div>
          </div>
        ...
        {success ? (
          <div className="p-6 space-y-4">
            <div className="flex items-center gap-2 text-emerald-700 font-black text-sm">
              <CheckCircle2 className="w-5 h-5" /> Mechanic account created successfully!
            </div>
            <div className="bg-slate-900 rounded-xl p-4 space-y-2">
              ...
              <div className="mt-3 pt-3 border-t border-slate-700">
                <p className="text-xs text-slate-500">⚠ Mechanic should change password after first login.</p>
              </div>
            </div>
            <button
            ...
            <button
              onClick={onClose}
              className="w-full bg-purple-500 hover:bg-purple-400 text-white font-black py-2.5 rounded-lg text-sm"
            >
              Done
            </button>
          </div>
          ...
          <form onSubmit={handleSubmit} className="p-6 space-y-4">
            ...
            <div>
              <label className="block text-xs font-bold text-slate-600 mb-1">Specialties</label>
              <input
            ...
            <div>
              ...
              <div className="flex gap-2">
                <div className="relative flex-1">
                  <input
                    ...
                    required
                    className="w-full border border-slate-200 rounded-lg px-3 py-2 pr-10 text-sm font-mono focus:outline-none focus:ring-2 focus:ring-purple-400"
                  />
              ...
              <p className="text-xs text-slate-400 mt-1">Mechanic will use this to log in. Remind them to change it.</p>
            </div>
            ...
            <div className="flex gap-3 pt-1">
              ...
              <button type="submit" disabled={loading}
                className="flex-1 bg-purple-500 hover:bg-purple-400 disabled:opacity-50 text-white font-black py-2.5 rounded-lg text-sm">
                {loading ? 'Creating...' : 'Create Mechanic Account'}
              </button>
            </div>
          </form>
        )}
