/**
 * Parse delivery barcodes — carrier tracking, JSON manifest QR, pipe/CSV manifest formats.
 * Returns normalized stop fields when parseable; never fabricates data.
 */

const US_STATE = /^[A-Z]{2}$/;

/** @typedef {{ tracking_number?: string, recipient_name?: string, recipient_phone?: string, address?: string, city?: string, state?: string, zip?: string, package_description?: string, notes?: string, barcode_format?: string, raw: string }} ParsedBarcode */
...
/** Pipe or tab manifest: tracking|name|address|city|state|zip|phone */
function tryDelimited(raw) {
  ...
  // Heuristic: if p3 looks like state and p4 like zip
  if (US_STATE.test(p4?.toUpperCase()) || /^\d{5}/.test(p5)) {
    return {
      tracking_number: p0,
      recipient_name: p1,
      address: p2,
...
/** Carrier tracking number patterns (generic — no affiliation with any carrier brand) */
