...
# Seeds demo data, opens phone-sized browser, prints walkthrough checklist.
...
Write-Host "   - FleetCo copyright + Patent Pending on login footer"
Write-Host "   - Allow Camera & Location (or Continue without camera)"
Write-Host ""
Write-Host "2. HOME DASHBOARD" -ForegroundColor Yellow
Write-Host "   - Quick tiles: Loads, Route, Scan, Dashcam, Navigate, HOS"
Write-Host "   - Today's delivery route card (3 stops seeded)"
Write-Host ""
Write-Host "3. ROUTE TAB" -ForegroundColor Yellow
Write-Host "   - Map with numbered pins (OSM, no Google API)"
Write-Host "   - Expand stop -> Navigate (OSM) + Record POD"
Write-Host "   - Optimize route button"
Write-Host ""
Write-Host "4. SCAN TAB" -ForegroundColor Yellow
Write-Host "   - Deliver / Build Route / Log modes"
Write-Host "   - F12 -> Network -> Offline to test offline queue"
Write-Host "   - Scan shows 'Saved on device' when offline"
Write-Host ""
Write-Host "5. OFFLINE SYNC" -ForegroundColor Yellow
Write-Host "   - Amber banner: 'Offline — activity saved on device'"
Write-Host "   - Uncheck Offline -> auto-sync banner"
Write-Host "   - Route reloads from server after sync"
Write-Host ""
Write-Host "6. CLOCK TAB (More menu)" -ForegroundColor Yellow
Write-Host "   - Clock in/out works offline, syncs later"
Write-Host ""
...
Write-Host ""
Write-Host "8. NAVIGATION TAB (More menu)" -ForegroundColor Yellow
